...
class ImportData(models.Model):
    ...
    def _compute_fetch_selection(self):
        cached_data = self.env.context.get('cached_import_data')
        if cached_data:
            _logger.info('Using cached data for fetch selection.')
            return json.loads(cached_data)  # Load JSON data back to Python object
        ...
        try:
            ...
            if data.get('resultCd') != '000':
                _logger.error('Error fetching selection data: %s', data.get('resultMsg'))
                return []

            item_list = data.get('data', {}).get('itemList', [])
            selection_data = [
                {
                    'id': f"{item['taskCd']}_{item['itemSeq']}",
                    'display_name': f"{item['itemNm']} - {item['taskCd']} - {item['orgnNatCd']}"
                }
                for item in item_list
            ]

            # Set the fetched data in a new context
            return self.with_context(cached_import_data=json.dumps(selection_data))._compute_fetch_selection()

        except requests.exceptions.RequestException as e:
            _logger.error('Failed to fetch selection data: %s', e)
            return []
