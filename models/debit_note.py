...
class AccountMove(models.Model):
    ...
    def action_debit_note_create(self):
        for move in self:
            if move.state == 'draft':
                return {
                    'type': 'ir.actions.act_window',
                    'name': 'Create Debit Note',
                    'res_model': 'debit.note.wizard',
                    'view_mode': 'form',
                    'target': 'new',
                    'context': {
    ...
    def create_debit_note_api_call(self):
        api_url = "http://localhost:8085/trnsSales/saveSales"
        payload = self.create_debit_note_payload()
    ...
    def _process_moves_debit(self):
        ...
        for move in self:
            ...
            result_msg = self.create_debit_note_api_call()
            move.message_post(body=f"API Response Debit Note resultMsg: {result_msg}")
                ...
                "itemList": [
                    {
                        ...
                        "splyAmt": round(line.quantity * self.calculate_tax_inclusive_price(line), 2),
                        "totDcAmt": 0,
                        "taxblAmt": round(line.price_subtotal, 2),
                        "vatCatCd": self.get_tax_description(self.get_primary_tax(partner)),
                        "iplCatCd": "IPL1",
                        "tlCatCd": "TL",
                        "exciseTxCatCd": "EXEEG",
            ...
            result_msg_new_endpoint = self._post_to_debit_api('http://localhost:8085/stock/saveStockItems',
                                                        payload_new_endpoint, "Save Stock Item API Response")
            self.message_post(body=f"API Response New Endpoint: {result_msg_new_endpoint}")
            ...
            result_msg_stock = self._post_to_debit_api('http://localhost:8085/stockMaster/saveStockMaster', payload_stock,
                                                 "Save Stock Master Endpoint response:")
            self.message_post(body=f"Endpoint response: {result_msg_stock}")
            # print(f"Save Stock Master Endpoint response: {result_msg_stock}")

    def _post_to_debit_api(self, url, payload, success_message_prefix):
        try:
